...
class result {
  ...
  public:
    ...
    OkType& get_ok_ref() { return std::get<OkType>(variant_); }

    /// @brief Throws on invalid access
    const OkType& get_ok_ref() const { return std::get<OkType>(variant_); }

    /// @brief Throws on invalid access
    ErrorType& get_err_ref() { return std::get<ErrorType>(variant_); }

    /// @brief Throws on invalid access
    const ErrorType& get_err_ref() const { return std::get<ErrorType>(variant_); }
